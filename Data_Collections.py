    ...
    if bboxs:
        ...
        for bbox in bboxs:
            ...
            if score > confidence:
                ...
                # Use a try-except block to handle potential timeouts or errors during blur calculation
                try:
                    blurValue = int(cv2.Laplacian(imgFace, cv2.CV_64F).var())
                except Exception as e:
                    print(f"Error in blur calculation: {e}")
                    blurValue = 0  # Default to 0 if there's an error
